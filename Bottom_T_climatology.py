...
clim_data = read_climatologies(file_path = '/home/guanl/Desktop/MSP/Climatology', output_folder = 'T_spr', season = 'spr')

clim_data_1 = bottom_values(clim_data)

#plot climatology on triangle grid
plot_clim_triangle(clim_data_1, file_path = '/home/guanl/Desktop/MSP/Climatology', left_lon = -160, right_lon = -102, bot_lat = 25, top_lat = 62, output_folder = 'T_spr', season = 'spr', depth = 'bottom')

# Convert and plot climatology on regular grid
left_lon, right_lon, bot_lat, top_lat = [-140, -120, 45, 56]

clim_data_r = triangle_to_regular(clim_data_1, file_path = '/home/guanl/Desktop/MSP/Climatology', left_lon = -140, right_lon = -120, bot_lat = 45, top_lat = 56, output_folder = 'T_spr', season = 'spr', depth = 'bottom')

# Convert to raster layer and save in .tif format
convert_to_tif(clim_data_r, file_path = '/home/guanl/Desktop/MSP/Climatology', output_folder = 'T_spr', season = 'spr', depth = 'bottom')

#use EEZ polygon to clip on GeoTiff
EEZ_clip(file_path = '/home/guanl/Desktop/MSP/Climatology', output_folder = 'T_spr', season = 'spr', depth = 'bottom')
...
def plot_clim_triangle(data_dict, file_path, left_lon, right_lon, bot_lat, top_lat, output_folder, season, depth):
    ...
    plt.show()
    png_name = os.path.join(file_path, output_folder + '/T_' + season + '_tri_' + depth + '.png')
...
def convert_to_tif(data_dict, file_path, output_folder, season, depth):
    ...
    tif_name = os.path.join(file_path, output_folder + '/T_' + season + '_' + depth + '.tif')
...
def EEZ_clip(file_path, output_folder, season, depth):
    tif_name = os.path.join(file_path, output_folder + '/T_' + season + '_' + depth + '.tif')
    tif_name_mask = os.path.join(file_path, output_folder + '/T_' + season + '_' + depth + '_masked.tif')
...
for i in clim_data['grid_depth']:
    var = clim_data['grid_depth_' + str(int(i)) + 'm']
    print(i)
    print(var.max())
